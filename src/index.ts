...
import { terrain, ITerrainKeys, IMap } from './configs/sprites'
...
locality.addEventListener('load', () => {
    const { map } = world
    map.forEach((row: Array<Array<ITerrainKeys>>, y: number) => {
        row.forEach((cell, x) => {
...
function move() {
    switch (key) {
        case 'ArrowDown':
            direction = 0
            if (pY >= widthField - spriteW) return
            pY += 10
            break
        case 'ArrowLeft':
            direction = spriteW
        ...
        case 'ArrowRight':
            direction = spriteW * 2
            if (pX >= widthField - spriteW) return
            pX += 10
            break
        case 'ArrowUp':
            direction = spriteW * 3
